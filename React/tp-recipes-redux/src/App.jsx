...
const OonSubmitFormInputHandler= async (e)=>{
  ...
  e.preventDefault()
  let BASE_URL = "https://m2i-demo-recipes-fox-default-rtdb.europe-west1.firebasedatabase.app/"
  if (isLoggin){
  ...
  try{
    ...
    if(!response.ok){
      throw new Error(`il a eu une erreur lors ${isLoggin? "du Log In" : "de l'enregistrement"}`)
    }
        ...
        >
          <div className="container-fluid">
            ...
            <div className="collapse navbar-collapse" id="navbarContacts">
              {isLogged ? (
                <button className="ms-auto btn btn-secondary" onClick={LogOutHandler}>Log out</button>
              ) : (
                <>
                  <button className="ms-auto btn btn-outline-info" onClick={() => modalLoginHandler("login")}>
                    Log In
                  </button>
                  <button className="ms-2 btn btn-primary" onClick={modalLoginHandler}>Register</button>
                </>
              )}
            </div>
          </div>
